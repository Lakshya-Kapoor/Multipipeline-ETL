...
import com.example.multiPipelineEtl.db.MongoJdbcConfig;
import com.example.multiPipelineEtl.db.MongoJdbcConnectionFactory;
...
public class PipelineOrchestrator {
    ...
    public void execute(ExecutionRequest request) {
        ...
        try (Connection postgresConnection = PostgresConnectionFactory.openConnection(postgresConfig);
            ...
            Query1Pipeline query1Pipeline = new MongoDbQuery1Pipeline(mongoConnection);
            query1Pipeline.run(request.getNumberOfBatches(), executionContext);
            out.println("MongoDB Query 1 execution completed.");
        } catch (SQLException ex) {
            throw new IllegalStateException("Failed to execute MongoDB Query 1 pipeline over JDBC.", ex);
